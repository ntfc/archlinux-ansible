...
We first need to update the submodule containing the AUR plugin.
...
=== Create host_vars vault

TODO: write about `ansible-vault create` and the `vault-pwd` file to be manually created
=== Configuration files
...
The `01-system.yml` will install components not specific to any user but rather used by the overall system.
You can run it with any of the following variants:

[source,shell]
----
# list available tasks. note the tag are shown in square brackets
